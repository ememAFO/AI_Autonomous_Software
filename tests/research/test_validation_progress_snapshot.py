...
def test_validation_progress_snapshot_flags_evidence_repair_after_ready_state():
    generator, registry, evidence_log = make_generator(
        registry_path=(
            "reports/intelligence/"
            "test_validation_progress_snapshot_repair_state_registry.json"
        ),
        evidence_log_path=(
            "reports/intelligence/"
            "test_validation_progress_snapshot_repair_evidence_log.json"
        ),
    )
    ...
    for index in range(5):
        evidence_log.add_entry(
            ...
            source_reference=f"interview_template_{index}",
            signal_strength="strong",
            supports_validation=True,
    ...
    registry.transition(
        ...
        policy_version=POLICY_VERSION,
        run_id="validation_progress_snapshot_repair_001",
    )
    ...
    snapshot = generator.generate(
        ...
        theme_name=THEME_NAME,
        policy_version=POLICY_VERSION,
        run_id="validation_progress_snapshot_repair_002",
    )

    assert snapshot.current_state == "READY_FOR_REVIEW"
    assert snapshot.evidence_status == "EVIDENCE_NEEDS_VERIFICATION"
    assert snapshot.gate_status == "REVIEW_REQUIRES_EVIDENCE_REPAIR"
    assert "previously passed the validation gate" in snapshot.gate_reason
    assert "Do not approve MVP planning" in snapshot.recommended_next_action
